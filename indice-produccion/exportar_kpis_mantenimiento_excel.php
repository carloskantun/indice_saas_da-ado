...
header('Content-Disposition: attachment; filename=kpis_mantenimiento.csv');
...
fputcsv($output, ['Total Reportes', $total]);
fputcsv($output, ['Pendientes', $pendientes]);
fputcsv($output, ['En Proceso', $proceso]);
fputcsv($output, ['Terminados', $terminados]);
fputcsv($output, ['Cancelados', $cancelados]);
fputcsv($output, ['Vencidos', $vencidos]);
