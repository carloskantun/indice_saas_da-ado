...
// 98 KPIs
$ordenes_totales = $conn->query("SELECT COUNT(*) AS total FROM ordenes_servicio_cliente")->fetch_assoc()['total'];
$ordenes_pagadas = $conn->query("SELECT COUNT(*) AS total FROM ordenes_servicio_cliente WHERE estatus = 'Pagado'")->fetch_assoc()['total'];
$ordenes_por_liquidar = $conn->query("SELECT COUNT(*) AS total FROM ordenes_servicio_cliente WHERE estatus = 'Por pagar'")->fetch_assoc()['total'];
$ordenes_vencidas = $conn->query("SELECT COUNT(*) AS total FROM ordenes_servicio_cliente WHERE estatus = 'Vencido'")->fetch_assoc()['total'];
...
$total_ordenes = $conn->query("SELECT COUNT(*) AS total FROM ordenes_servicio_cliente WHERE 1=1")->fetch_assoc()['total'];
...
<div class="col-12 col-md-4">
    ...
    <select class="form-select select2-multiple" id="delegado" name="delegado[]" multiple="multiple">
        ...
        <?php endwhile; ?>
                    ...
                    <a href="minipanel_servicio_cliente.php" class="btn btn-outline-secondary">Limpiar Filtros</a></div>
...
<!-- 98 Tabla de Órdenes de Servicio al Cliente -->
...
<td class="col-ver_pdf">
  <a href="generar_pdf_servicio_cliente.php?folio=<?php echo $orden['folio']; ?>" target="_blank" class="btn btn-sm btn-outline-dark">Ver PDF</a>
</td>
                </tr>
            <?php endwhile; ?>
        </tbody>
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    document.querySelectorAll(".estatus-select").forEach(select => {
        select.addEventListener("change", function () {
            ...
            fetch("actualizar_estatus_servicio_cliente.php", {
                method: "POST",
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    document.querySelectorAll(".quien-realizo-select").forEach(select => {
        select.addEventListener("change", function () {
            actualizarCampo("actualizar_quien_realizo_servicio_cliente.php", this.dataset.id, "quien_realizo_id", this.value);
        });
    });
    ...
    document.querySelectorAll(".nivel-select").forEach(select => {
        select.addEventListener("change", function () {
            actualizarCampo("actualizar_nivel_servicio_cliente.php", this.dataset.id, "nivel", this.value);
        });
    });
});
</script>
    ...
    document.addEventListener("DOMContentLoaded", function () {
        ...
        if (botonVerMas) {
            botonVerMas.addEventListener("click", function () {
                ...
                fetch("cargar_servicio_cliente.php?pagina=" + pagina)
                    .then(response => response.text())
                    .then(data => {
...
<script>
    ...
    modalCompletar.addEventListener("show.bs.modal", function (event) {
        ...
        fetch("completar_servicio_cliente.php?folio=" + folio)
            .then(res => res.text())
            .then(html => {
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  cargarContenidoModal("modalIngresarOrden", "ordenes_servicio_cliente.php?modal=1", "contenidoOrden");
});
</script>
<script>
document.addEventListener("DOMContentLoaded", function () {
...
if (thOrden) {
    ...
    if (dir) filtros.set("dir", dir);
        ...
        window.open("exportar_servicio_cliente_pdf.php?" + filtros.toString(), "_blank");
    });
});
document.addEventListener("DOMContentLoaded", function () {
    ...
    btnExportarCSV.addEventListener("click", function () {
        ...
        window.open("exportar_servicio_cliente.php?" + filtros.toString(), "_blank");
    });
});
</script>
<script>
document.addEventListener("DOMContentLoaded", function () {
    function aplicarFiltroRapido(estatus) {
        ...
        window.location.href = "minipanel_servicio_cliente.php?" + params.toString();
    }
