...
$mes_terminado = obtener("SELECT COUNT(*) AS total FROM ordenes_servicio_cliente $where AND estatus = 'Terminado' AND DATE_FORMAT(fecha_reporte, '%Y-%m') = '$mes'", $conn);
$cumplimiento_mes = $mes_total > 0 ? round(($mes_terminado / $mes_total) * 100, 1) : 0;

// Órdenes por mes
$q1 = $conn->query("SELECT DATE_FORMAT(fecha_reporte, '%Y-%m') AS mes, COUNT(*) AS total FROM ordenes_servicio_cliente $where GROUP BY mes ORDER BY mes");
...
// Costo mensual
$q2 = $conn->query("SELECT DATE_FORMAT(fecha_reporte, '%Y-%m') AS mes, SUM(costo_final) AS total FROM ordenes_servicio_cliente $where GROUP BY mes ORDER BY mes");
...
// Unidades
$q3 = $conn->query("SELECT u.nombre AS unidad, COUNT(*) AS total FROM ordenes_servicio_cliente o JOIN unidades_negocio u ON o.unidad_negocio_id = u.id $where GROUP BY o.unidad_negocio_id");
...
foreach ($estatus_labels as $e) {
    $estatus_valores[] = obtener("SELECT COUNT(*) AS total FROM ordenes_servicio_cliente $where AND COALESCE(NULLIF(TRIM(estatus), ''), 'Pendiente') = '$e'", $conn);
}

// Completadas por día
$where_completadas = ["estatus = 'Terminado'", "fecha_completado IS NOT NULL"];
if (!empty($_GET['alojamiento'])) {
    $ids = implode(',', array_map('intval', $_GET['alojamiento']));
...
if ($fecha_inicio && $fecha_fin) {
    ...
    $res = $conn->query("SELECT DATE(fecha_completado) AS dia, COUNT(*) AS total FROM ordenes_servicio_cliente $where_comp_sql GROUP BY dia ORDER BY dia ASC");
...
$q_users = $conn->query("SELECT (SELECT nombre FROM usuarios WHERE id = quien_realizo_id) AS usuario, COUNT(*) AS total FROM ordenes_servicio_cliente $where_comp_sql GROUP BY quien_realizo_id");
...
$top_pendientes = [];
$res = $conn->query("SELECT a.nombre, COUNT(*) AS total FROM ordenes_servicio_cliente o JOIN alojamientos a ON o.alojamiento_id = a.id $where AND COALESCE(NULLIF(TRIM(o.estatus), ''), 'Pendiente') = 'Pendiente' GROUP BY o.alojamiento_id ORDER BY total DESC LIMIT 5");
while ($row = $res->fetch_assoc()) {
    $top_pendientes[] = ['nombre' => $row['nombre'], 'total' => (int)$row['total']];
}

$top_terminados = [];
$res = $conn->query("SELECT a.nombre, COUNT(*) AS total FROM ordenes_servicio_cliente o JOIN alojamientos a ON o.alojamiento_id = a.id $where AND COALESCE(NULLIF(TRIM(o.estatus), ''), 'Pendiente') = 'Terminado' GROUP BY o.alojamiento_id ORDER BY total DESC LIMIT 5");
while ($row = $res->fetch_assoc()) {
